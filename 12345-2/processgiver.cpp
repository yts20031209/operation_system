...
std::vector<process> processGiver::CreatProcess(int num) {
    ...
    for (int i = 1; i <= num; i++) {
        ...
        double space=static_cast<double>(std::rand())/RAND_MAX * 10.0;
        double ran=(static_cast<double>(std::rand())/RAND_MAX * 10.0)-7.5;
        double spaces=space+ran;
        spacex+=spaces;
        process process(id, priority, runTime,space,spacex);
